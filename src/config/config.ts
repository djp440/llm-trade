...
class ConfigLoader {
    ...
    private static loadConfig(): AppConfig {
        ...
        try {
            ...
            console.warn(`Warning: Could not load config.toml from ${configPath}, using defaults or failing.`);
            throw error;
        }
        ...
        return {
            exchange: {
                ...
                isSandbox,
                apiKey: isSandbox ? process.env.DEMO_API_KEY || '' : process.env.PROD_API_KEY || '',
                apiSecret: isSandbox ? process.env.DEMO_API_SECRET || '' : process.env.PROD_API_SECRET || '',
                apiPassword: isSandbox ? process.env.DEMO_API_PASSWORD : process.env.PROD_API_PASSWORD,
            },
            llm: {
